...
* **Layering**: two separate SVG overlays, `.bfly-track--trail` (z-index 0, behind section content) and `.bfly-track--fly` (z-index 2, above everything). The trail is the only thing occluded by cards/photos/tiles; the butterfly itself is always fully visible. Both share one viewBox so they stay registered to the same coordinates.
* **Landing**: the path's last two anchors bring the butterfly in on a shallow approach and perch it on the top edge of the "Submit an inquiry" button (not on its face - the glyph's greens match the button fill, `#3B6D11`, so landing on top of it would hide the butterfly).
