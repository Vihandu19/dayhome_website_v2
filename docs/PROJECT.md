...
* **Layering**: two separate SVG overlays sharing one viewBox. `.bfly-track--trail` (z-index 1) sits above all normal page content - headings, cards, tiles, badges - and is only hidden behind the two elements individually promoted above it: `.about-img-placeholder` and `.cta-section .btn-primary` (both z-index 2). `.bfly-track--fly` (z-index 3, the butterfly itself) always sits above everything, trail included. Sections deliberately carry no `position`/`z-index` of their own - see "Layering model" below for why.
* **Landing**: the path's last anchors ease the meander's centerline and decay its oscillation toward the landing point (a smoothstep over the final 40% of the flight), so the approach is one continuous curve rather than a separate hard-coded detour. It perches at the top-right corner of the "Submit an inquiry" button, a few px into its face so the body visually rests on the surface.
...
**Layering model - don't give `.site > section` a blanket `z-index`.** An earlier version gave every section `position: relative; z-index: 1` so opaque section backgrounds could occlude the trail (and made `.hero`/`.info-bar`/`.cta-section` transparent to let it show through). That's gone. Setting `z-index` on a section creates a stacking context that traps the section's whole subtree as one unit - no `z-index` on a descendant inside it can out-rank an element outside it, since `z-index` cannot escape an ancestor's stacking context. That made it impossible for "trail above text but below one specific button inside the same section" to exist. The fix was to leave sections as normal, unpositioned content (so the trail, a positioned z-index element, paints above them by ordinary CSS painting-order rules) and individually promote only `.about-img-placeholder` and `.cta-section .btn-primary` above the trail's z-index. If a future change needs a section to occlude something again, promote that specific element - don't put `z-index` back on the section.

**Measurement note**: `butterfly.js` measures nav and the CTA button with `getBoundingClientRect` relative to `.site`, not `offsetTop`. `offsetTop` resolves against `offsetParent`, which changes depending on which ancestor happens to be positioned - it broke once already when a section briefly had `position: relative`. `getBoundingClientRect` is immune to that regardless of what stacking changes happen above it later - don't switch it back.
